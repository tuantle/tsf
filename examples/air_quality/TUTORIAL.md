...
The training strategy is to look back at the previous 2 hours of input data and make forecast for next hour **PM25** pollution level.
...
Here is the model output result for the validation dataset. The expected and predicted **PM25** level are plot on top of each other. This might look like a case of overfitting since the prediction is very well matched with the expectation, but this is not the dataset that the model was trained with. This is the validation dataset, so the model is doing a decent job at making predictions for next hour pollution level.
